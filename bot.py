...
def webhook_oxapay():
    """Webhook seguro para OxaPay con validación HMAC-SHA256"""
    data = request.json
...
def webhook_cryptomus():
    """Webhook seguro para Cryptomus con validación MD5 + IP whitelist"""
...
def webhook_nowpayments():
    """Webhook seguro para NOWPayments con validación HMAC-SHA512"""
    data = request.json
...
def index():
    return redirect('https://t.me/' + config.TELEGRAM_BOT_USERNAME.replace('@', ''))

@app.route('/pay', methods=['GET', 'POST'])
def pay_landing():
    """Landing page pública para pedir el ID de Telegram"""
    if request.method == 'POST':
        user_id = request.form.get('user_id')
        if user_id and user_id.isdigit():
            return redirect(f'/pay/{user_id}')
    ...
    <head>
        ...
        <title>V-Strike VIP - Acceso</title>
    ...
    <body class="bg-gray-900 text-white min-h-screen flex items-center justify-center p-4">
        <div class="bg-gray-800 p-8 rounded-xl shadow-2xl w-full max-w-sm border border-gray-700 text-center">
            ...
            <p class="text-gray-400 mb-8">Ingresa tu ID de Telegram para continuar</p>
            
            <form method="POST" class="space-y-4">
                <input type="number" name="user_id" placeholder="Ej: 12345678" required 
                    class="w-full bg-gray-700 border border-gray-600 rounded p-3 text-center text-white focus:outline-none focus:border-blue-500 placeholder-gray-500">
                
                <button type="submit" class="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 px-6 rounded-lg transition transform hover:scale-105 shadow-lg">
                    Continuar al Pago ➡️
                </button>
            </form>
            <p class="mt-6 text-xs text-gray-500">¿No sabes tu ID? Escribe /start en el bot.</p>
        </div>
    </body>
    </html>
    """
...
def pay_page(user_id):
    """Página pública de pago para usuarios externos (fuera de Telegram)"""
    # Link de Deep Linking para abrir el bot y pagar con tarjeta
    # Reemplaza 'VStrike1690_Bot' con la variable de config si está disponible
    bot_username = config.TELEGRAM_BOT_USERNAME.replace('@', '')
    telegram_pay_link = f"https://t.me/{bot_username}?start=pay"
    
    stripe_enabled = bool(getattr(config, 'STRIPE_PUBLISHABLE_KEY', None))
    stripe_html = ""
    if stripe_enabled:
        stripe_html = f'''
                <form action="/pay/stripe/process" method="POST" class="mt-4">
                    <input type="hidden" name="user_id" value="{user_id}">
                    <button type="submit" class="block w-full bg-slate-100 hover:bg-white text-gray-900 font-bold py-4 px-6 rounded-lg transition transform hover:scale-105 shadow-lg flex items-center justify-center gap-2">
                        <span>💳</span> Pagar con Tarjeta (Directo Web)
                        <span class="text-xs font-normal text-gray-500">(Stripe Seguro)</span>
                    </button>
                </form>
        '''
    ...
    <body class="bg-gray-900 text-white min-h-screen flex items-center justify-center font-sans p-4">
        <div class="bg-gray-800 p-6 md:p-8 rounded-xl shadow-2xl w-full max-w-sm border border-gray-700 text-center">
            ...
            <div class="space-y-4">
                <!-- Opción 1: Tarjeta vía Telegram (Stars) -->
                <a href="{telegram_pay_link}" class="block w-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-4 px-6 rounded-lg transition transform hover:scale-105 shadow-lg flex items-center justify-center gap-2 no-underline">
                    <span>💳</span> Pagar con Tarjeta / Apple Pay
                    <span class="text-xs font-normal opacity-75">(Vía Telegram)</span>
                </a>

                <!-- Opción Stripe (solo si está configurado) -->
                {stripe_html}
            ...
            <p class="mt-6 text-xs text-gray-500">
                El acceso se activa automáticamente para el ID: {user_id}
            </p>
        </div>
    </body>
    </html>
...
def pay_process():
    """Procesa el clic en el botón de pago"""
    user_id = request.form.get('user_id')
    if not user_id:
    ...
    return "Error generando el pago. Intenta de nuevo más tarde."


@app.route('/pay/stripe/process', methods=['POST'])
def pay_stripe_process():
    """Procesa el clic en Pagar con Stripe"""
    user_id = request.form.get('user_id')
    if not user_id:
        return "Error: User ID missing", 400
    ...
    # Generar link Stripe
    result = generate_payment_link("Stripe", amount, order_id, int(user_id))
    
    if result:
        # Registrar transacción
        db.create_transaction(order_id, int(user_id), amount, "Stripe", "Web Direct")
        
        # Redirigir a la sesión de Checkout
        url = result.get('url')
        if url:
             from flask import redirect
             return redirect(url)
    
    return "Error generando el pago con Stripe. Verifica la configuración."
...
def process_successful_payment(order_id):
    """Lógica común para procesar pagos exitosos"""
    try:
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status_text = "🌟 VIP Activo" if is_vip else "❌ Usuario Gratuito"
...
async def pay(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [
        [InlineKeyboardButton("⭐ Estrellas (Rápido)", callback_data='pay_Stars')],
        [InlineKeyboardButton("🪙 Cripto / Tarjeta (NOWPayments)", callback_data='pay_NOWPayments')],
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text(f"Elige tu método de pago (Precio: ${config.VIP_PRICE_USD} USD):", reply_markup=reply_markup)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if method == "Stars":
        ...
        # Generar link para otros métodos (NOWPayments, etc)
        await query.edit_message_text(f"Generando enlace de pago para {method}...")
        
        # payments.generate_payment_link maneja la lógica
        ...
        if link_data:
            ...
            keyboard = [[InlineKeyboardButton("🔗 Pagar Ahora", url=url)]]
            reply_markup = InlineKeyboardMarkup(keyboard)
            
            await query.edit_message_text(
                f"Has elegido **{method}**.\n"
                f"Usa el botón abajo para completar el pago.\n"
...
async def successful_payment_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(chat_id=config.ADMIN_ID, text=f"💰 Nueva venta vía Stars!\nUsuario: {user_id}")
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Solo notificar al admin si es algo crítico, para evitar spam
    if update and update.effective_user and update.effective_user.id == config.ADMIN_ID:
         await context.bot.send_message(chat_id=config.ADMIN_ID, text=f"⚠️ Error: {context.error}")
...
async def post_init(application: Application):
    """Inicialización asíncrona después de que la app arranque"""
    # Asegurar que no hay webhook para que el polling funcione
    logger.info("Deleting webhook to ensure polling works...")
    await application.bot.delete_webhook(drop_pending_updates=True)
    
    # Iniciar tarea de puente (Bridge) en segundo plano
    asyncio.create_task(bridge_task(application.bot))
    logger.info("Bridge task started.")
...
def main():
    ...
    application = Application.builder().token(config.TELEGRAM_BOT_TOKEN).request(request_kwargs).post_init(post_init).build()
    ...
    print("--- VERSION FIX 2.0 ---")
    
    # Run the bot (Handles loop, signals, and cleanup automatically)
